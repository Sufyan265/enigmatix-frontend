...
const SuperAdminReports = () => {
    ...
    const { superAdminReportData, superAdminReport, loading } = useReportContext();
            ...
            <div className="container mx-auto bg-white shadow-md rounded-lg">
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full leading-normal">
                        ...
                        <tbody>
                            {superAdminReportData.map((report, index) => (
                                <tr key={index} className="bg-white border-b border-gray-200 hover:bg-gray-50">
                                    <td className="py-4 px-6 text-left">{report.company.name}</td>
                                    <td className="py-4 px-6 text-right text-green-500 font-semibold">${report.totalIncome.toLocaleString()}</td>
                                    <td className="py-4 px-6 text-right text-red-500 font-semibold">${report.totalExpenses.toLocaleString()}</td>
                                    <td className="py-4 px-6 text-right text-gray-700 font-bold">$ {(report.totalIncome - report.totalExpenses).toFixed(2)} </td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
